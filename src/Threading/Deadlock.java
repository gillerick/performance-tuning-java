...
public class Deadlock implements Runnable {
  ...
  public void init(String name, Deadlock friend) {
    //We have a name, and a reference to the other Deadlock object so that we can call each other
    me = name;
    other = friend;
  }

  public static void main(String[] args) {
    //wait as long as the argument suggests(or use 20ms as default)
    int wait = args.length == 0 ? 20 : Integer.parseInt(args[0]);

    Deadlock d1 = new Deadlock();
    Deadlock d2 = new Deadlock();

    //make sure that the Deadlock objects know each other
    d1.init("d1", d2);
    d1.init("d2", d1);
    ...
    //Start the first thread, then wait as long as instructed before starting the other
    d1Thread.start();
    try {
      Thread.sleep(wait);
    } catch (InterruptedException e) {
  ...
  public synchronized void run() {
    ...
    System.out.println("Calling hello from " + me + other.me);
    other.hello();
